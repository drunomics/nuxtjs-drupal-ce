  ...
  actions: {
    ...
    async getPage ({ commit }, { path, config, context, options, axios }) {
      try {
        ...
        // Ensure the response looks valid. Either title and content keys
        ...
        if (data.redirect) {
          if (process.server) {
            context.redirect(data.redirect.url)
          } else {
            window.onNuxtReady(() => { window.$nuxt.$router.push(data.redirect.url) })
          }
        } else {
