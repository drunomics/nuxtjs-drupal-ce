class DrupalCe {
  ...
  async fetchPage (path, config = {}) {
    await this.context.store.dispatch('drupalCe/getPage', { path, config, context: this.context, options: this.options })
  }
  ...
  async fetchMenu (name, config = {}) {
    await this.context.store.dispatch('drupalCe/getMenu', { name, config, context: this.context })
  }
...
const storeModule = {
  ...
  actions: {
    ...
    async getMenu ({ commit }, { name, config, context }) {
    ...
    async getPage ({ commit }, { path, config, context, options }) {
      try {
        config.params = config.params ?? {}
