...
/**
 * Helper function to compare the fetched page with the current page in store.
 * 
 * The page object fetched using drupalCe/getPage action
 * @param {Object} fetchedPage
 * 
 * The page object in the store
 * @param {Object} currentPage
 * 
 * @returns {boolean}
 */
async function comparePageObject (fetchedPage, currentPage) {
  const keys = Object.keys(currentPage)
  const hasChanged = keys.some(key => { 
    return currentPage[key] !== fetchedPage[key]}
  )
  return hasChanged
}
...
const storeModule = {
  ...
  actions: {
    ...
    async getPage ({ commit, state }, { path, config, context, options, axios }) {
      try {
        config.params = config.params ?? {}
        ...
        if (!(data.title || data.constent) && !data.redirect) {
        ...
        // Do not redirect if the current URL is the same as the redirect URL.
        if (data.redirect && (options.currentURL !== data.redirect.url)) {
          ...
          // Using the comparePageObject helper function to compare the fetched page
          const pageChanged = await comparePageObject(data, state.page)
          // Only mutate the state if the content sent by the server is different
          // from the one already in the store, avoiding unnecessary payload to the client.
          if (pageChanged) {
            commit('page', data)
          }
...
export default function (context, inject) {
  ...
  // Make sure to apply the baseURLs from runtime-config if set.
  const runtimeConfig = $config['drupal-ce'] || {}
  options.baseURL = runtimeConfig.baseURL || options.baseURL
  options.frontendBaseURL = runtimeConfig.frontendBaseURL || options.frontendBaseURL
  options.currentURL = options.frontendBaseURL + context.route.path
