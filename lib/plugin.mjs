...
const errorHandler = (error, commit, context, options) => {
  if (error.response) {
    // Check if error comes from getMenu action
    if (error.from === "getMenu") {
      commit('addMessage', {
        type: 'error',
    ...
    else if (!options.customErrorPages && error.response.data && error.response.data.title && error.response.data.content) {
  ...
  actions: {
    ...
    async getMenu ({ commit }, { name, config, context, options, axios }) {
      ...
      try {
        ...
        Object.assign(error, {from: "getMenu"});
        errorHandler(error, commit, context, options )
      }
    },
    async getPage ({ commit }, { path, config, context, options, axios }) {
      try {
        ...
        Object.assign(error, {from: "getPage"});
        errorHandler(error, commit, context, options)
      }
    }
  }
