  ...
  actions: {
    ...
    async getPage ({ commit }, { path, config, context, options, axios }) {
      try {
        ...
        if (data.redirect) {
          // If the URL contains '://', then it must be an absolute URL,
          // therefore we don't need to add the base URL to it.
          if (data.redirect.url.indexOf('://') > 0) {
            // Redirect to the absolute URL.
            return context.redirect(data.redirect.url)
          }
          if (process.server) {
