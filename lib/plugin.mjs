...
Vue.filter('decodeSpecialChars', (value) => {
  ...
  const encodedAttr = /&(?:#039|#39|#8364|#60|#62|#10);/g
  return value.replace(encodedAttr, match => decodingMap[match])
})

const pageErrorHandler = (error, commit, context, options) => {
  if (error.response) {
    // Request made and server responded. If we get a proper Drupal
    // error page, apply it and propagate the error code.
...
const menuErrorHandler = (error, commit, context) => {
  if (error.response) {
    commit('addMessage', {
      type: 'error',
      message: `Menu error: ${error.message}.`
    })
  } else if (error.request) {
    // The request was made but no response was received. Usually this
    // means a network error.
  ...
  actions: {
    ...
    async getMenu ({ commit }, { name, config, context, options, axios }) {
      ...
      try {
        ...
        options.menuErrorHandler(error, commit, context, options)
      }
    },
    async getPage ({ commit }, { path, config, context, options, axios }) {
      try {
        ...
        options.pageErrorHandler(error, commit, context, options)
      }
    }
  }
}
...
export default function (context, inject) {
  ...
  Object.assign(options, <%= serialize(options) %>);
  ...
  options.pageErrorHandler = options.pageErrorHandler || pageErrorHandler
  options.menuErrorHandler = options.menuErrorHandler || menuErrorHandler
